...
class TelaEditarServico(QtWidgets.QMainWindow):
    ...
    def setupUi(self):
        ...
        self.botaoHelp.clicked.connect(
            lambda: help("Ajuda - Editar Serviço", HELPEDITARSERVICO)
        )
    ...
    def getServico(self):
        ...
        if not (
            self.lineEditValorServico.text().replace(",", "", 1).isnumeric()
            or self.lineEditValorServico.text().replace(".", "", 1).isnumeric()
        ):
            raise Exception('Campo "valor" inválido!')
        if (
            -Decimal(self.lineEditValorServico.text().replace(",", ".", 1))
            .as_tuple()
            .exponent
            > 2
        ):
