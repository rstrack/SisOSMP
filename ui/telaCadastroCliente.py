...
class TelaCadastroCliente(QtWidgets.QMainWindow):
    ...
    def retranslateUi(self):
        ...
        self.labelTitulo.setText(_translate(
            "MainWindow", "Cadastro de Clientes"))
    ...
    def salvar(self):
        try:
            ...
            elif veiculo: r = self.clienteCtrl.salvarVeiculo(veiculo)
    ...
    def getDadosCliente(self):
        ...
        if (self.lineEditCPFJ.text()):
            if self.comboBoxPessoa.currentIndex() == 0:
                cpfj = 'CPF'
                if len(self.lineEditCPFJ.text()) != 11:
                    raise Exception('CPF inválido')
            else:
                cpfj = 'CNPJ'
                if len(self.lineEditCPFJ.text()) != 14:
                    raise Exception('CNPJ inválido')
            if not self.lineEditCPFJ.text().isnumeric():
                raise Exception(f'Digite apenas números no campo {cpfj}')
            dict['documento'] = self.lineEditCPFJ.text()
        else:
            dict['documento'] = None
