...
class TelaEditarOrcamento(QtWidgets.QMainWindow):
    ...
    def setupUi(self):
        ...
        self.botaoHelp.clicked.connect(
            lambda: help("Ajuda - Editar Orçamento", HELPEDITARORCAMENTO, 450)
        )
    ...
    def aprovarOrcamento(self):
        try:
            ...
            if isinstance(r, Exception):
                raise Exception(r)
            r = OrcamentoController.aprovarOrcamento(self.orcamentoID)
            if isinstance(r, Exception):
                raise Exception(r)
            msg = QtWidgets.QMessageBox()
            msg.setWindowIcon(QtGui.QIcon("resources/logo-icon.png"))
    ...
    def reprovarOrcamento(self):
        try:
            ...
            if msgBox.clickedButton() == y:
                r = OrcamentoController.reprovarOrcamento(self.orcamentoID)
                if isinstance(r, Exception):
                    raise Exception(r)
                msg = QtWidgets.QMessageBox()
                msg.setWindowIcon(QtGui.QIcon("resources/logo-icon.png"))
