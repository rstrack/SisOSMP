...
class InfiniteScrollTableModel(QtCore.QAbstractTableModel):
    ...
    def colunasDesejadas(self, colunas: list):
        ...
        for dict in self._data:
            self._data[self._data.index(dict)] = {
                key: (dict[key] if key in dict else None) for key in colunas
            }
        self.endResetModel()

    def setHeaderData(
        self, section: int, orientation: QtCore.Qt.Orientation, value, role: int = ...
    ) -> bool:
        if orientation == QtCore.Qt.Orientation.Horizontal and role in (
    ...
    def data(self, index, role=QtCore.Qt.ItemDataRole.DisplayRole):
        ...
        if (
            index.row() >= self.numRows
            or index.row() < 0
            or index.column() >= self.numColumns
            or index.column() < 0
        ):
            return QtCore.QVariant()
        ...
        if role == QtCore.Qt.ItemDataRole.DisplayRole:
            return QtCore.QVariant(
                list(self._data[index.row()].values())[index.column()]
            )
    ...
    def canFetchMore(self, index):
        ...
        index=QModelIndex
        """
        if self.numRows < len(self._data) or self.numColumns < len(
            self._data[0].keys()
        ):
            return True
        else:
    ...
    def fetchMore(self, index):
        ...
        if rowsToFetch > 0:
            self.beginInsertRows(
                QtCore.QModelIndex(), self.numRows, self.numRows + rowsToFetch - 1
            )
            self.endInsertRows()
