...
class TelaEditarPeca(QtWidgets.QMainWindow):
    ...
    def setupUi(self):
        ...
        self.botaoHelp.clicked.connect(
            lambda: help("Ajuda - Editar Peça", HELPEDITARPECA)
        )
    ...
    def getPeca(self):
        ...
        if not (
            self.lineEditValorPeca.text().replace(",", "", 1).isnumeric()
            or self.lineEditValorPeca.text().replace(".", "", 1).isnumeric()
        ):
            raise Exception('Campo "valor" inválido!')
        if (
            -Decimal(self.lineEditValorPeca.text().replace(",", ".", 1))
            .as_tuple()
            .exponent
            > 2
        ):
    ...
    def renderEditar(self, id):
        ...
        self.lineEditValorPeca.setText(
            "{:.2f}".format(peca["valor"]).replace(".", ",", 1)
        )
